...
def smoke_test():
    ...
    """
    initial, final_train, val_loss, val_acc = run_once(epochs=2, batch_size=128, subset_train=2000, seed=0, device="cpu")
...
def main():
    ...
    initial, final_train, val_loss, val_acc = run_once(
        epochs=args.epochs, batch_size=args.batch_size, subset_train=subset_train, seed=args.seed, device=str(device)
    )
